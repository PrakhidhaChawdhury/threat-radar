...
async def scrape_url(collector_id: str, url: str) -> Optional[list[dict]]:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=20.0, follow_redirects=True) as client:
            ...
            # If HackerNews Developer Threats (Algolia JSON Search API)
            if "hnrss.org" in url or "ycombinator.com" in url or "algolia.com" in url:
                hn_api = "https://hn.algolia.com/api/v1/search_by_date?tags=story&query=scam+OR+malicious+OR+phishing+OR+vulnerability&hitsPerPage=20"
                resp = await client.get(hn_api, headers=custom_headers)
                if resp.status_code == 200:
                    ...
                    for hit in hits:
                        ...
                        story_text = hit.get("story_text") or title or ""
                        parsed_items.append({
                            "url": story_url,
                            "title": title,
                            "content": story_text if len(story_text) > 20 else title,
